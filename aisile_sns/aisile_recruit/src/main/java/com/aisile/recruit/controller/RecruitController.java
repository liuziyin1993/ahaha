package com.aisile.recruit.controller;
import java.util.List;
import java.util.Map;

import entity.MyConstants;
...
import com.aisile.recruit.service.RecruitService;
...
public class RecruitController {
	...
	public Result findByRecommend(){
		List<Recruit> list = recruitService.findTop4ByStateOrderByCreatetimeDesc(MyConstants.RECRUIT_RECOMMENT);
		return new Result(true,StatusCode.OK,"查询推荐职位成功",list);
	}

	/**
	 * 查询前12个  以创建日期降序   state 不为0
	 * @param
	 */
	@RequestMapping(value="/search/newlist",method= RequestMethod.GET)
	public Result newList(){
		List<Recruit> list = recruitService.findTop12ByStateNotOrderByCreatetimeDesc("0");
		return new Result(true,StatusCode.OK,"查询最新职位成功",list);
	}
